...
（32-byte nonce、单次消费、bounded attempts）、`device_sessions`（token hash、absolute expiry、
每 device 至多一个 active session）、`device_revocation_requests`（owner-scoped idempotency +
不可变结果快照）。Gateway 不查询 Core/Runtime/Reliability 表；`workos_core.devices`（migration
...
operator workosctl device pair → Gateway admin Unix socket（0600，仅此一服务，TCP 确定性 404）
  → RotatePairingTicket：owner 锁内 revoke 旧 pending + 插入新 ticket
  → QR URL：https://<origin>/pair#v=1&t=<43-char base64url>&fp=sha256:<leaf DER digest>
  → 浏览器：WebCrypto 生成 non-extractable P-256 private key（IndexedDB only）
  → BeginPairing（claim pending ticket，绑定 key digest/name/class）
...
（SameSite=Strict 只是纵深防御）；auth 端点解码前 16 KiB、admin socket 4 KiB；auth 端点叠加
不信任 `X-Forwarded-For` 的 bounded RemoteAddr rate limiter（map 有容量与淘汰上限），对象级
attempt 预算持久化于 DB；store 故障是净化 503（绝不伪装 401，也不回退 configured identity）；
...
请求稳定 `Aborted`；revoke + 全部 session + 快照同事务；撤销的 `revoked_at` 用数据库事务时间。
...
`clients/device-auth/src/transcript.ts`）。验收门禁 `make test-lan-pairing`：临时 TLS leaf +
