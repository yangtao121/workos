...
无真实账号内容或外部服务。Knowledge 混合 review/workspace 结果；System Monitor 非空遥测和服务 unavailable。after 改进来源文案、窄屏表格滚动与换行摘要，并保留失败说明和重试入口。

```sh
docker run --rm --network host --user "$(id -u):$(id -g)" -e HOME=/tmp \
  -e WORKOS_E2E_URL=http://127.0.0.1:5173 \
  -e WORKOS_SERVICE_CAPTURE_DIR=/workspace/docs/ui/desktop-web/changes/20260906-service-views/after \
  -v "$PWD:/workspace" -w /workspace/apps/desktop-web \
  workos-playwright:1.62.1 node node_modules/@playwright/test/cli.js test service-views-visual.spec.ts --workers=1
```
