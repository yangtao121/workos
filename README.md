...
| Artifact | workos-core | `working` | two immutable subtypes with full real-stack evidence: web bundle (owner-scoped launch payload, bounded upload, canonical digest, durable idempotency, private assets — never public bytes) and project review artifacts (ADR-0008, migration 021): canonical document.markdown.v1 and code.unified-diff.v1 materialized through the private lease-bound AppendTaskArtifact RPC — Core derives owner/project/task from the active task lease only, normalizes and bounds content (CRLF→LF, NUL/C0/C1 rejection, ≤512 KiB/20k lines/16 KiB per line), mints UUIDv7/digest/time, adjudicates the durable (task, output key) identity in one transaction with the Core-minted ArtifactCreated timeline event (concurrency, response-loss replay, type-slot conflicts, lease-loss and restart convergence on real PostgreSQL), never trusts provider-built artifact references (generic AppendTaskEvent fails closed), publishes owner/project-scoped ListArtifacts and typed GetReviewArtifact with digest recomputation (corruption→Internal, transient→Unavailable, web bundle→not-reviewable), and renders read-only inert Markdown/unified-diff windows (Artifact Center + clickable timeline events) in the desktop shell (make test-artifact-review real cross-process Chromium gate + deterministic before/after/current visual records + real-PostgreSQL integration tests); Fake provider declares exact supported artifact types with deterministic output, DeepSeek/Generic CLI honestly report unsupported; generic artifact storage beyond these subtypes unimplemented |
