...
| Indexer | indexer | `working` | project review-artifact durable lexical index/search (ADR-0013, migrations 026/027): Core appends exactly one content-free indexing publication in the same transaction as each immutable review artifact and each project archive tombstone (tx-scoped sink, rollback/replay evidence); the private IndexPublicationSourceService arbitrates at-least-once claims with database leases (FOR UPDATE SKIP LOCKED, stale complete never acks, expired leases re-claimable) and serves bounded resolve/reconciliation pages through a neutral source authority over the Artifact and Project modules; the indexer owns the workos_index schema (generation-scoped lexical documents, publication receipts, consumer cursor, durable repair jobs, shadow generations) and its at-least-once worker commits document+receipt+cursor locally before completing Core, so response-loss replay is a receipt no-op (real-PostgreSQL tests: same-transaction rollback, double claimant, lease expiry, archive race, terminal corruption verdict); owner search through the Gateway is bounded deterministic lexical (simple-config tsquery, versioned snapshot page tokens bound to owner/project/query/ranking/generation/snapshot, pure-function excerpts, sanitized error matrix) with foreign-project and oversized-query fail-closed evidence (make test-project-knowledge-search over the real stack + Chromium); operator rebuild via the local admin socket and workosctl index status/rebuild/job drives a durable requested->snapshotting->catching_up->validating->promoting machine whose promotion is a single-row CAS (golden-equivalence, crash-resume, idempotency replay/conflict, cancel, and destroy-and-restore golden equivalence in a temporary indexer-owned database with Core rows untouched: make test-project-knowledge-rebuild); Knowledge Center window and Knowledge Lab fixture with deterministic before/after/current visual records. Not covered: generic archive/object store, semantic RAG/embeddings/pgvector (both unavailable by fixed reason), workspace/file sources, multi-owner deployments |
