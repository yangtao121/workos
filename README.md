...
| Notification | workos-core | `working` | owner-scoped durable notification facts + monotonic change stream + monotonic read state + App quota/idempotency receipts (ADR-0014, migration 029; reliability publication migration 030 owner reliability-host): Core atomic producers join their source transactions through a neutral tx-scoped sink (approval required, first task terminal, review artifact created — a notification failure rolls the source back; replay/digest drift is physical-receipt arbitrated; real-PostgreSQL rollback + exactly-once + concurrency evidence), reliability incident facts arrive at-least-once over the versioned private claim/complete RPC (lease + FOR UPDATE SKIP LOCKED, lost-completion replay is a receipt no-op, same-source/different-digest fails closed, outage degrades only incident freshness and recovery drains the backlog) with make test-incident-notifications proving the real cross-process chain (runtime observation -> reliability publication -> Core projection) while explicitly NOT proving rootless supervisor acceptance, gateway exposes exactly the public NotificationService (list/get/read/summary + resumable watch with bounded lifetime, heartbeats, per-owner stream budgets, sweep-gap RESET_REQUIRED, pre-decode wire budgets) with dynamic device-session identity injection and periodic mid-stream session revalidation so a revoked session terminates its stream in bounded time, two real Chromium contexts prove live delivery, paired-device read convergence from the READ change alone, typed actions re-reading their authoritative targets, and durability across a Gateway/Core restart (make test-notification-center + restart-battery notifications-seed/verify), and the granted-app notifications.create chain (capability vocabulary + consent grant + runtime bridge method negotiated from the exact grant epoch + Core private ingest with single-transaction idempotency/quota/projection, replay-first, burst + UTC daily caps atomic in PostgreSQL, revoke fail-closed with zero side effects) is proven by make test-app-notifications over an opaque-origin web bundle (make test-app-notifications). Not covered: APNs/FCM/Web Push background delivery (foreground + durable catch-up only), Service Worker push, native wrappers, notification text search, per-user preference/quiet hours |
