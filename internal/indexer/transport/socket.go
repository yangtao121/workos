...
func ListenAdminSocket(path string, adminHandler http.Handler, logger *slog.Logger) (*AdminSocket, error) {
	if path == "" || !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return nil, errors.New("indexer admin socket path must be absolute and cleaned")
	}
	...
	if stat, err := os.Lstat(path); err == nil {
		if stat.Mode()&os.ModeSocket == 0 {
			return nil, fmt.Errorf("indexer admin socket path %s exists and is not a Unix socket", path)
		}
		...
		case statErr != nil:
			return nil, fmt.Errorf("recheck stale indexer admin socket: %w", statErr)
		case current.Mode()&os.ModeSocket == 0 || !os.SameFile(stat, current):
			return nil, fmt.Errorf("indexer admin socket %s changed while checking staleness", path)
		default:
			if err := os.Remove(path); err != nil {
				return nil, fmt.Errorf("remove stale indexer admin socket: %w", err)
			}
			logger.Info("removed stale indexer admin socket", "path", path)
		}
	} else if !errors.Is(err, os.ErrNotExist) {
		return nil, fmt.Errorf("inspect indexer admin socket: %w", err)
	}
	listener, err := net.Listen("unix", path)
	if err != nil {
		return nil, fmt.Errorf("listen indexer admin socket: %w", err)
	}
	unixListener, ok := listener.(*net.UnixListener)
	if !ok {
		_ = listener.Close()
		return nil, errors.New("indexer admin listener is not a Unix listener")
	}
	unixListener.SetUnlinkOnClose(false)
	bound, err := os.Lstat(path)
	if err != nil || bound.Mode()&os.ModeSocket == 0 {
		closeOwnedSocket(listener, path, bound)
		return nil, errors.New("bound indexer admin socket path is unavailable")
	}
	if err := os.Chmod(path, 0o600); err != nil {
		closeOwnedSocket(listener, path, bound)
		return nil, fmt.Errorf("chmod indexer admin socket: %w", err)
	}
	pathInfo, err := os.Lstat(path)
	if err != nil || pathInfo.Mode()&os.ModeSocket == 0 || !os.SameFile(bound, pathInfo) {
		closeOwnedSocket(listener, path, bound)
		return nil, errors.New("indexer admin socket path changed while setting permissions")
	}
...
func validateSocketParent(path string) error {
	...
	if errors.Is(err, os.ErrNotExist) {
		if filepath.Clean(filepath.Dir(parent)) != "/" {
			return errors.New("indexer admin socket parent must be a single controlled directory")
		}
		if err := os.Mkdir(parent, 0o700); err != nil {
			return fmt.Errorf("create indexer admin socket directory: %w", err)
		}
		info, err = os.Lstat(parent)
	}
	if err != nil {
		return fmt.Errorf("indexer admin socket parent is unavailable: %w", err)
	}
