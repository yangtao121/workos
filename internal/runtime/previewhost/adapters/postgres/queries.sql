...
VALUES(sqlc.arg(preview_id),sqlc.arg(owner_user_id),sqlc.arg(project_id),sqlc.arg(idempotency_key),sqlc.arg(workspace_source_id),sqlc.arg(read_only),sqlc.arg(state),sqlc.arg(created_at),sqlc.arg(updated_at),sqlc.arg(expires_at),sqlc.arg(command),sqlc.arg(port),sqlc.arg(generation),sqlc.arg(access_token),sqlc.arg(request_digest),sqlc.arg(binding_id),sqlc.arg(binding_revision),sqlc.arg(lifecycle_mode)) ON CONFLICT DO NOTHING;
...
UPDATE workos_runtime.workspace_previews SET generation=generation+1,state='queued',updated_at=$3,expires_at=$4,lifecycle_mode=$5 WHERE owner_user_id=$1 AND preview_id=$2 RETURNING *;
