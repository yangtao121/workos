...
compose stop model
python3 tools/real-model-acceptance/budget_proxy.py "$WORKOS_V2_MODEL_PORT" "$WORKOS_V2_DIR/budget.jsonl" &
proxy_pid=$!
repo=$(pwd)
docker run --rm --network host --user "$WORKOS_V2_USER" --group-add "$WORKOS_V2_DOCKER_GID" -v /usr/bin/docker:/usr/local/bin/docker:ro -v /var/run/docker.sock:/var/run/docker.sock -e HOME=/tmp -e GOPATH=/tmp/workos-go -e GOMODCACHE=/go/pkg/mod -e GOCACHE=/tmp/go-cache -e GOPROXY=off -v "$repo:$repo" -w "$repo" -v workos-go-cache:/go/pkg/mod -v "$repo/tmp/go-build-cache:/tmp/go-cache" -e WORKOS_REAL_MODEL_GATE_URL="http://127.0.0.1:$WORKOS_V2_GATEWAY_PORT" -e WORKOS_REAL_MODEL_GATE_PROJECT="$WORKOS_V2_PROJECT_ID" -e WORKOS_REAL_MODEL_GATE_WORKSPACE="$WORKOS_V2_DIR/project" golang:1.26.7-bookworm go test -tags='integration realmodelgate' -count=1 -run '^TestRealModelDeepSeekContinuousSession$' -v -timeout 20m ./tests/integration
printf 'A15 PASS: live DeepSeek, two native development turns. Budget ledger: %s/budget.jsonl\n' "$WORKOS_V2_DIR"
