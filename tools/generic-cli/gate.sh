...
cleanup() {
  ...
  if [ "$result" -eq 0 ]; then printf 'test-generic-cli: PASS (isolated Core/Harness/CLI artifacts, context and health)\n'; fi
  exit "$result"
}
trap cleanup EXIT
trap 'exit 130' INT
...
for attempt in $(seq 1 60); do
  if curl --noproxy '*' --silent --fail --max-time 2 -H 'Content-Type: application/json' -d '{}' "http://127.0.0.1:$WORKOS_CLI_GATE_GATEWAY_PORT/workos.harness.v1.HarnessCatalogService/GetHarnessCatalog" > "$task_dir/catalog.json"; then ready=true; break; fi
  sleep 1
done
[ "$ready" = true ]
docker run --rm --network host --user "$WORKOS_CLI_GATE_USER" -e HOME=/tmp -e PLAYWRIGHT_BROWSERS_PATH=/ms-playwright -e WORKOS_E2E_URL="http://127.0.0.1:$WORKOS_CLI_GATE_GATEWAY_PORT" -e WORKOS_CLI_GATE_EXECUTABLE="/workspace/${task_dir#"$repo/"}/cli/run" -e WORKOS_E2E_OUTPUT_DIR="/workspace/${task_dir#"$repo/"}/results" -v "$repo:/workspace" -w /workspace/apps/desktop-web "${E2E_IMAGE:-workos-playwright:1.62.1}" node node_modules/@playwright/test/cli.js test generic-cli.spec.ts --workers=1
